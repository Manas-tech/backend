...
async function convertDetailedPurchasesToServiceConsumption() {
  ...
  try {
    ...
    // Convert purchases to service consumption records with detailed mapping
    const serviceConsumptions = purchases.map((purchase, index) => {
      ...
      // Estimate duration based on service category and amount
      let duration = 60; // Default 1 hour
      if (purchase.serviceCategory === 'Social Media') {
        duration = 120; // 2 hours for social media
      } else if (purchase.serviceCategory === 'Development') {
        duration = 480; // 8 hours for development
      } else if (purchase.serviceCategory === 'Design') {
        duration = 180; // 3 hours for design
      } else if (purchase.serviceCategory === 'Marketing') {
      ...
      return {
        ...
        serviceId: purchase.serviceId || purchase.stripeProductId || `service_${index}`,
        serviceName: purchase.serviceName || 'Unknown Service',
        serviceCategory: purchase.serviceCategory || 'General',
        consumptionType: consumptionType,
        quantity: 1,
        duration: duration,
        status: status,
        startDate: purchase.startDate || purchase.purchasedAt || new Date(),
        endDate: status === 'completed' ? new Date((purchase.startDate || purchase.purchasedAt || new Date()).getTime() + (duration * 60 * 1000)) : null,
        notes: purchase.notes || `Converted from purchase record. Original amount: ${purchase.totalAmount || 'N/A'} ${purchase.currency || 'USD'}`,
        metadata: {
...
convertDetailedPurchasesToServiceConsumption();
